...
// lets try it via recursion and tabulation without sp opt 
    ...
    bool solve(vector<int>&nums, int ind, int target, vector<vector<int>>&dp)
    {
        ...
        bool take = nums[ind] <= target ? solve(nums, ind-1, target-nums[ind], dp) : false; 
        bool nottake = solve(nums, ind-1, target, dp); 
